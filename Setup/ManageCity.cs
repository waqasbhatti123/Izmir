...
namespace FOS.Setup
{
    public class ManageCity
    {
                    ...
                    cityData = dbContext.Tbl_IZBillingPeriod
                            .OrderByDescending(x => x.IsActive).ToList().Select(
                                u => new IZMonthCloseData
                                {
                                    ID = u.ID,
                                    Name = u.Name,
                                    //ReadingStart = u.ReadingStart.Value.ToString("dd-MMM-yyyy"),
                                    //IssueDate = u.BillIssueDate.Value.ToString("dd-MMM-yyyy"),
        ...
        public static List<IZBillingParaMeterData> GetBillingParamter()
        {
            List<IZBillingParaMeterData> ConData = new List<IZBillingParaMeterData>();
            try
            {
                using (FOSDataModel dbContext = new FOSDataModel())
                {

                    ConData = dbContext.Tbl_IZBillingParameter
                            .ToList().Select(
                                u => new IZBillingParaMeterData
                                {
                                    ID = u.ParaID,
                                    Domestic = u.Domestic,
                                    DomesticTemp = u.DomesticTemp,
                                    Commercial = u.Commercial,
                                    Society = u.Soceity,
                                    TeleCommunication = u.TeleCommunication,
                                    FPA = u.FPA
                                }).ToList();
        ...
        public static List<IZHomeData> GetReadingByMCOForGrid(int MonthID)
        {
            List<IZHomeData> ConData = new List<IZHomeData>();
            List<IZHomeData> Data = new List<IZHomeData>();
            try
            {
                ...
                using (FOSDataModel dbContext = new FOSDataModel())
                {
                    ...
                    List<sp_GetReadingbyMCO_Result> cur = dbContext.sp_GetReadingbyMCO(MonthID).ToList();
                    ...
                    foreach (var item in cur)
                    {
                        ...
                        hoData.Adress = item.Address;
                        hoData.MeterNo = item.MeterNo;
                        hoData.CurrentReading = Convert.ToDecimal(item.Reading);
                        hoData.PreReading = Convert.ToDecimal(item.PreReading);
                        hoData.UnitConsume = item.AvgUnit;
                        hoData.ReadingFeadback = item.Remarks;
                        hoData.Image = item.img;
                        Data.Add(hoData);
                    }
        ...
        public static List<IZHomeData> GetSummaryReadingForGrid()
        {
            List<IZHomeData> ConData = new List<IZHomeData>();
            List<IZHomeData> Data = new List<IZHomeData>();
            int MonhID;
            try
            {
                ...
                using (FOSDataModel dbContext = new FOSDataModel())
                {
                    Tbl_IZBillingPeriod bil = dbContext.Tbl_IZBillingPeriod.Where(x => x.IsActive == true).FirstOrDefault();
                    
                    MonhID = bil.ID;

                    List<sp_GetMeterReadingPreCurrentMonth_Result> cur = dbContext.sp_GetMeterReadingPreCurrentMonth(MonhID).ToList();

                    foreach (var item in cur)
                    {
                        var hoData = new IZHomeData();
                        hoData.ID = Convert.ToInt32(item.ID);
                        hoData.BlockName = item.BlockName;
                        hoData.MonthName = item.MonthName;
                        hoData.CurrentReading = Convert.ToDecimal(item.Reading);
                        hoData.PreMonthName = item.PreMonth;
                        hoData.PreReading = Convert.ToDecimal(item.PreReading);
                        hoData.Percentage = Convert.ToDecimal(item.Percentage);
                        hoData.CurrentConnect = Convert.ToInt32(item.CurrentMonthConnection);
                        hoData.PreviConnect = Convert.ToInt32(item.PreviousConnection);
                        Data.Add(hoData);
                    }
        ...
        public static List<IZHomeData> GetAboveUniReadingForGrid(int unit)
        {
            List<IZHomeData> ConData = new List<IZHomeData>();
            List<IZHomeData> Data = new List<IZHomeData>();
            int MonhID;
            try
            {
                ...
                using (FOSDataModel dbContext = new FOSDataModel())
                {
                    ...
                    List<sp_GetReadingAboveUnit_Result> cur = dbContext.sp_GetReadingAboveUnit(unit,MonhID).ToList();
        ...
        public static List<IZBillingParaMeterData> GetResultBillingPara(string search, string sortOrder, int start, int length, List<IZBillingParaMeterData> dtResult, List<string> columnFilters)
        {
            return FilterResultBillingPara(search, dtResult, columnFilters).SortBy(sortOrder).Skip(start).Take(length).ToList();
        }
        ...
        {
            return FilterMcoResultCurrentMonth(search, dtResult, columnFilters).SortBy(sortOrder).Skip(start).Take(length).ToList();
        }

        public static List<IZHomeData> GetMcoReadingCurrentMonth(string search, string sortOrder, int start, int length, List<IZHomeData> dtResult, List<string> columnFilters)
        {
            return FilterResultCurrentMonth(search, dtResult, columnFilters).SortBy(sortOrder).Skip(start).Take(length).ToList();
        }

        public static List<IZHomeData> GetSummaryReadingCurrentMonth(string search, string sortOrder, int start, int length, List<IZHomeData> dtResult, List<string> columnFilters)
        {
            return FilterSummaryResultCurrentMonth(search, dtResult, columnFilters).SortBy(sortOrder).Skip(start).Take(length).ToList();
        }

        public static List<IZHomeData> GetAboveUnitReadingCurrentMonth(string search, string sortOrder, int start, int length, List<IZHomeData> dtResult, List<string> columnFilters)
        {
            return FilterAboveUnitResultCurrentMonth(search, dtResult, columnFilters).SortBy(sortOrder).Skip(start).Take(length).ToList();
        }
        ...
        public static int CountBilliPara(string search, List<IZBillingParaMeterData> dtResult, List<string> columnFilters)
        {
            return FilterResultBillingPara(search, dtResult, columnFilters).Count();
        }
        ...
        public static int CountMCOCurrentMonthReading(string search, List<IZHomeData> dtResult, List<string> columnFilters)
        {
            return FilterMcoResultCurrentMonth(search, dtResult, columnFilters).Count();
        }

        public static int CountSummaryCurrentMonthReading(string search, List<IZHomeData> dtResult, List<string> columnFilters)
        {
            return FilterSummaryResultCurrentMonth(search, dtResult, columnFilters).Count();
        }

        public static int CountAboveUnitCurrentMonthReading(string search, List<IZHomeData> dtResult, List<string> columnFilters)
        {
            return FilterAboveUnitResultCurrentMonth(search, dtResult, columnFilters).Count();
        }
        ...
        private static IQueryable<IZBillingParaMeterData> FilterResultBillingPara(string search, List<IZBillingParaMeterData> dtResult, List<string> columnFilters)
        {
            IQueryable<IZBillingParaMeterData> results = dtResult.AsQueryable();

            results = results.Where(p => (search == null || (p.Domestic != null && p.Domestic.ToLower().Contains(search.ToLower())))
                );
        ...
        private static IQueryable<IZHomeData> FilterMcoResultCurrentMonth(string search, List<IZHomeData> dtResult, List<string> columnFilters)
        {
            IQueryable<IZHomeData> results = dtResult.AsQueryable();
        ...
        private static IQueryable<IZHomeData> FilterSummaryResultCurrentMonth(string search, List<IZHomeData> dtResult, List<string> columnFilters)
        {
            IQueryable<IZHomeData> results = dtResult.AsQueryable();
        ...
        private static IQueryable<IZHomeData> FilterAboveUnitResultCurrentMonth(string search, List<IZHomeData> dtResult, List<string> columnFilters)
        {
            IQueryable<IZHomeData> results = dtResult.AsQueryable();
